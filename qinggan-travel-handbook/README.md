...
终端会显示本地访问地址，通常为 `http://localhost:5173`。首次访问会播放 8 至 12 秒的旅行序章；播放完成或跳过后，浏览器会记录该状态，页头可随时重播。系统开启“减少动态效果”时会直接进入主内容。
...
未填写 Key、加载超时或高德服务暂不可用时，地图页会自动打开完整的本地图文路线。全部地点、四条参考路线、筛选、顺路组合、地点攻略和“已去过”切换仍可使用。Intro 的中国轮廓来自 `public/maps/china-outline.geojson`，不依赖高德先加载。
